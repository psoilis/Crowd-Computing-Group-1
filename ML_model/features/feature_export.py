...
def feature_export(input_file, output_file):
    with open(input_file, 'r', encoding='utf8') as f:
        ...
        for i in range(len(tweets)):
            ...
            feature_output = [str(tweets[i]['tweet_id']), str(tweet_len_words), str(tweet_len_chars),
                              str(tweet_punc_count), str(tweet_has_url), str(tweet_has_short_url),
                              str(tweet_has_emoji), str(tweet_has_mentions), str(tweet_num_mentions),
                              str(tweet_num_hashtags), str(tweet_num_likes), str(tweet_num_retweets),
                              str(tweet_num_replies), str(tweet_sentiment), str(tweet_is_reply),
                              str(user_time), str(user_followers), str(user_friends), str(user_is_verified),
                              str(user_handle_has_number)]
            # If first sample, write the file headers first
            if not headers:
                feature_headers = ['Tweet_ID', 'Tweet_Length_Words', 'Tweet_Length_Chars', 'Tweet_Punctuation_Count',
                                   'Tweet_Has_URL', 'Tweet_Has_Short_URL', 'Tweet_Has_Emoji', 'Tweet_Has_User_Mentions',
                                   'Tweet_Num_Mentions', 'Tweet_Num_Hashtags', 'Tweet_Num_Likes', 'Tweet_Num_Retweets',
                                   'Tweet_Num_Replies', 'Tweet_Compound_Score', 'Tweet_Is_Reply',
                                   'User_Time_on_Twitter', 'User_Followers', 'User_Friends', 'User_Is_Verified',
                                   'User_Handle_Has_Number']
                # Writing file headlines
                with open(output_file, encoding='utf8', mode='w', newline='') as features_file:
                    features_writer = csv.writer(features_file, quoting=csv.QUOTE_NONE, delimiter=',', quotechar='')
                    features_writer.writerow(feature_headers)
                headers = True

            with open(output_file, encoding='utf8', mode='a', newline='') as features_file:
                features_writer = csv.writer(features_file, quoting=csv.QUOTE_NONE, delimiter=',', quotechar='')
